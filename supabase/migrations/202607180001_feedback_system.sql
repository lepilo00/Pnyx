...
declare s feedback_surveys; inv feedback_invitations; sub_id uuid; q feedback_questions; val jsonb;
begin
 ...
 for q in select * from feedback_questions where survey_id=s.id and enabled loop val:=p_answers->q.question_key; if q.required and (val is null or val='null'::jsonb or val='""'::jsonb) then raise exception 'Required answer missing: %',q.question_key; end if; if val is not null and pg_column_size(val)>10000 then raise exception 'Answer too large'; end if; end loop;
 ...
 insert into feedback_answers(submission_id,question_id,question_key,value) select sub_id,q.id,q.question_key,p_answers->q.question_key from feedback_questions q where q.survey_id=s.id and q.enabled and p_answers?q.question_key;
