...
type GuideForm = Pick<Walk, 'title' | 'slug' | 'subtitle' | 'description' | 'location_name' | 'duration_minutes' | 'cover_image_url' | 'default_language' | 'completion_message' | 'bonus_section_title' | 'bonus_section_description' | 'is_published' | 'localized_content'> & { available_languages: string; display_mode: NonNullable<Walk['display_mode']> }
const empty: GuideForm = { title: '', slug: '', subtitle: '', description: '', location_name: '', duration_minutes: 30, cover_image_url: '', default_language: 'en', available_languages: 'en', completion_message: '', bonus_section_title: 'Bonus Stories', bonus_section_description: '', localized_content: {}, display_mode: 'stops', is_published: false }
...
export default function AdminGuidesPage() {
  ...
  const load = async () => { const { data, error: loadError } = await supabase.from('walks').select('*').order('title'); if (loadError) setError(loadError.message); else setGuides(data as Walk[]) }
  useEffect(() => {
  ...
  const save = async () => {
    const missing = [!form.title.trim() && 'Walk title', !form.slug.trim() && 'Slug', !(form.completion_message ?? '').trim() && 'Completion message'].filter(Boolean)
    ...
    const payload = { ...form, available_languages: form.available_languages.split(',').map((item) => item.trim()).filter(Boolean), subtitle: form.subtitle || null, cover_image_url: form.cover_image_url || null }
  ...
  const field = (key: keyof GuideForm, label: string, required = false) => <label className="block text-xs font-semibold text-stone-600">{label}<input required={required} value={String(form[key] ?? '')} onChange={(event) => setForm({ ...form, [key]: key === 'duration_minutes' ? Number(event.target.value) : event.target.value })} className="mt-1 w-full rounded-lg border border-stone-300 px-3 py-2 text-sm" /></label>
