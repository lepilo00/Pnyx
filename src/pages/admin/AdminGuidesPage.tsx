...
type GuideForm = Pick<Walk, 'title' | 'slug' | 'subtitle' | 'description' | 'location_name' | 'duration_minutes' | 'cover_image_url' | 'default_language' | 'stripe_product_id' | 'price' | 'completion_message' | 'bonus_section_title' | 'bonus_section_description' | 'is_published' | 'localized_content'> & { available_languages: string; display_mode: NonNullable<Walk['display_mode']> }
const empty: GuideForm = { title: '', slug: '', subtitle: '', description: '', location_name: '', duration_minutes: 20, cover_image_url: '', default_language: 'en', available_languages: 'en', stripe_product_id: '', price: 0, completion_message: '', bonus_section_title: 'Bonus Stories', bonus_section_description: '', localized_content: {}, display_mode: 'stops', is_published: false }
...
export default function AdminGuidesPage() {
  ...
  const save = async () => {
    const missing = [
      !form.title.trim() && 'Walk title',
      !form.slug.trim() && 'Slug',
      !(form.completion_message ?? '').trim() && 'Completion message',
    ].filter(Boolean)
