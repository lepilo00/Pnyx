...
export default function PremiumDiscoverCard({ imageSrc, title, description, to, stops }: PremiumDiscoverCardProps) {
  const content = (
    <article className="flex h-full bg-[#f4ecdf]/55 transition-colors group-hover:bg-[#f1e6d5]/75 dark:bg-stone-900/30 dark:group-hover:bg-stone-900/55">
      <PremiumImage src={imageSrc} containerClassName="w-28 shrink-0 self-stretch min-[600px]:w-32" imgClassName="h-full w-full object-cover" />
