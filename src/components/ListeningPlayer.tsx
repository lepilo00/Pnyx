...
export default function ListeningPlayer({ player, title, onPrevious, onNext }: Props) {
  ...
  const { currentTime, duration, isPlaying, isLoading, hasAudio, playbackRate, togglePlay, seek, skip, setPlaybackRate } = player
  const progress = duration ? currentTime / duration * 100 : 0
  const rates = [1, 1.25, 1.5, 2]
  return (
    <section className="fixed inset-x-0 bottom-0 z-40" aria-label={`Audio player: ${title}`}>
      <div className="relative mx-auto max-w-lg border-t border-amber-200 bg-parchment-50/95 px-4 pt-2 shadow-[0_-12px_35px_rgba(28,25,23,0.12)] backdrop-blur-xl dark:border-stone-700 dark:bg-stone-900/95" style={{ paddingBottom: 'calc(.6rem + env(safe-area-inset-bottom))' }}>
        ...
        <div className="mt-1 flex items-center justify-between">
          <button onClick={onPrevious} disabled={!onPrevious} className="h-10 w-10 text-stone-600 disabled:opacity-25" aria-label={t('listening.previousStory')}>‹</button>
          <button onClick={() => skip(-15)} className="h-10 w-10 text-xs font-bold text-stone-600" aria-label={t('listening.back15')}>−15</button>
          ...
          <button onClick={() => skip(15)} className="h-10 w-10 text-xs font-bold text-stone-600" aria-label={t('listening.forward15')}>+15</button>
          <button onClick={onNext} disabled={!onNext} className="h-10 w-10 text-stone-600 disabled:opacity-25" aria-label={t('listening.nextStory')}>›</button>
        </div>
        <button onClick={() => setPlaybackRate(rates[(rates.indexOf(playbackRate) + 1) % rates.length])} className="absolute bottom-[calc(.8rem+env(safe-area-inset-bottom))] right-4 rounded-full border border-stone-300 px-2 py-1 text-[10px] font-bold text-stone-600 dark:border-stone-600 dark:text-stone-300" aria-label={t('listening.playbackSpeed', { rate: playbackRate })}>{playbackRate}×</button>
      </div>
    </section>
  )
}
